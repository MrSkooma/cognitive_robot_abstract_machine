...
def test_add_multiple_columns_creates_lateral_connections_for_sum_nodes():
    ...
    assert progressive_pc.ppc.has_edge(column1.column_root, column2.column_root)


def test_add_column_renormalizes_older_column_that_gained_a_lateral_child():
    template = _make_mixture_template("x", "y")
    progressive_pc = ProgressivePC(template_pc=template)

    column1 = progressive_pc.add_column(task_id="task-a")
    progressive_pc.add_column(task_id="task-b")

    # column1's root gained column2's aligned node as a third child; its
    # weights must be renormalized so it still sums to a valid distribution.
    assert len(column1.column_root.subcircuits) == 3
    assert column1.column_root.is_normalized()
...
def test_iter_aligned_nodes_skips_lateral_sum_to_sum_connections():
    ...
    assert progressive_pc.ppc.has_edge(
        left_column.column_root, right_column.column_root
    )
...
def _make_indicator_mixture(
    ...
    """
    A mixture where every branch also carries a unique value of
    ``branch_variable``, so sibling branches never overlap in support and the
    mixture is deterministic.
    """
    pc = ProbabilisticCircuit()
    sum_root = SumUnit(probabilistic_circuit=pc)
    for element in branch_variable.domain.simple_sets:
        product = ProductUnit(probabilistic_circuit=pc)
        ...
        product.add_subcircuit(leaf(indicator, pc))
...
def _make_nested_layout() -> ProbabilisticCircuit:
    ...
    flat_product = ProductUnit(probabilistic_circuit=pc)
    for variable in outer_variables:
        flat_product.add_subcircuit(
            leaf(GaussianDistribution(variable=variable, location=0.0, scale=1.0), pc)
        )
    flat_product.add_subcircuit(
        leaf(
            SymbolicDistribution(
                variable=top_branch,
                probabilities=MissingDict(float, {hash("outer-0"): 1.0}),
            ),
            pc,
        )
    )
    sum_root.add_subcircuit(flat_product, log_weight=0.0)

    nested_product = ProductUnit(probabilistic_circuit=pc)
    for variable in outer_variables:
        nested_product.add_subcircuit(
            leaf(GaussianDistribution(variable=variable, location=0.0, scale=1.0), pc)
        )
    nested_product.add_subcircuit(
        leaf(
            SymbolicDistribution(
                variable=top_branch,
                probabilities=MissingDict(float, {hash("outer-1"): 1.0}),
            ),
            pc,
        )
    )
    inner_mixture = _make_indicator_mixture(inner_branch, inner_variables)
    remap = pc.mount(inner_mixture.root)
    nested_product.add_subcircuit(remap[inner_mixture.root.index])
    sum_root.add_subcircuit(nested_product, log_weight=0.0)
...
def _make_split_layout() -> ProbabilisticCircuit:
    ...
    left_branch = _make_indicator_variable("split_left", branch_count=2)
    left_variables = [Continuous(f"split_a{i}") for i in range(3)]
    left_mixture = _make_indicator_mixture(left_branch, left_variables)
    left_remap = pc.mount(left_mixture.root)
    root.add_subcircuit(left_remap[left_mixture.root.index])

    right_branch = _make_indicator_variable("split_right", branch_count=3)
    right_variables = [Continuous(f"split_b{i}") for i in range(3)]
    right_mixture = _make_indicator_mixture(right_branch, right_variables)
    right_remap = pc.mount(right_mixture.root)
    root.add_subcircuit(right_remap[right_mixture.root.index])

    return pc


def _any_sum_unit(progressive_pc: ProgressivePC, column: PCColumn) -> SumUnit:
    """Any one SumUnit belonging to the given column, regardless of layout shape."""
    return next(
        node
        for node in progressive_pc.get_column_nodes(column)
...
# %% lateral-connection experiment
@pytest.mark.parametrize(
    "make_layout",
    [_make_wide_layout, _make_nested_layout, _make_split_layout],
    ids=["wide", "nested", "split"],
)
def test_lateral_connections_preserve_decomposability_but_break_determinism(
    make_layout,
):
    ...
    progressive_pc = ProgressivePC(template_pc=template)
    progressive_pc.add_column(task_id="task-a")
    assert progressive_pc.ppc.is_decomposable()
    assert progressive_pc.ppc.is_deterministic()

    progressive_pc.add_column(task_id="task-b")
    progressive_pc.add_column(task_id="task-c")

    # The lateral SumUnit-to-SumUnit connections added between columns never
    # touch a ProductUnit's children, so decomposability is unaffected.
    assert progressive_pc.ppc.is_decomposable()
    # Those same lateral connections make a receiving SumUnit's new sibling
    # cover the same variable domain as its own branches, so their supports
    # overlap and determinism breaks as soon as a second column is added.
    assert not progressive_pc.ppc.is_deterministic()

    # Only an older column's SumUnits receive a lateral child (from each
    # newer column added afterwards); the newest column's own SumUnits never
    # gain one, so they stay locally deterministic in isolation.
    oldest_sum_unit = _any_sum_unit(progressive_pc, progressive_pc.columns[0])
    newest_sum_unit = _any_sum_unit(progressive_pc, progressive_pc.columns[-1])
    assert not oldest_sum_unit.is_deterministic()
    assert newest_sum_unit.is_deterministic()
